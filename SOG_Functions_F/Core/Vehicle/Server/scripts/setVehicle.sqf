...

{
	...
	if (isNil "_disableFlag" && _parentVehicleType in sog_allowed_veh) then {
		...
		if (isNil "_vehicleUID") then {
			_sql_res = call compile ("extDB3" callExtension format["0:SQL:INSERT INTO vehicles SET vehicleType = '%1', vehiclePos = '%2', vehicleDir = '%3', vehicleFuel = '%5', vehicleDamage = '%6', mission_FK = '%4', vehicleClass = '%7', vehicleName = '%8'", _vehicleType, _vehiclePos, _vehicleDir, sog_mission_fk, _vehicleFuel, _vehicleDamage, _vehicleClass, _vehicleName]);
		...
		_null = [_x, _vehicleUID] execVM "\SOG_Server\SOG_Functions_F\Vehicle\Server\scripts\setVehicleInventory.sqf";
		...
		_sql_res = call compile ("extDB3" callExtension format["1:SQL:UPDATE vehicles SET vehicleType = '%1', vehiclePos = '%2', vehicleDir = '%3', vehicleFuel = '%5', vehicleDamage = '%6', mission_FK = '%4', vehicleClass = '%7', vehicleName = '%8', vehicleHitpoints = '%10', vehicleCrew = '%11', rec_date = NOW() WHERE id = '%9'", _vehicleType, _vehiclePos, _vehicleDir, sog_mission_fk, _vehicleFuel, _vehicleDamage, _vehicleClass, _vehicleName, _vehicleUID, _vehicleHitpointDamage, _vehicleCrew]);
	};
} forEach (vehicles);