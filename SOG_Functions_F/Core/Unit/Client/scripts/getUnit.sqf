...
_playerAdmin = "0";
diag_log format ["SOG - Loading profile for player %1", _playerName];
if ((count _sql_res) > 35) then {
	...
	if(!isNil {
		...
		if ((count _playerInventory) > 5) then {
			diag_log format ["SOG - Setting inventory for player %1", _playerName];
			_Player setUnitLoadout _playerInventory;
			diag_log format ["SOG - Setting inventory for player %1 complete", _playerName];
		};
	};
};
...
diag_log format ["SOG - Loading profile for player %1 complete", _playerName];